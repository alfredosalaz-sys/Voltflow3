...
// Capa 1: Google Places API  -> nombre, direcciÃ³n, rating, web, telÃ©fono, horario
// Capa 2: Web Scraping PRO   -> emails, redes, decisor, descripciÃ³n, JSON-LD, Schema.org
// Capa 3: Hunter.io          -> verificaciÃ³n y bÃºsqueda de emails corporativos
// ============================================================
  ...
  el.innerHTML = `
    <div style="display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;flex-wrap:wrap">
      <div>
        ...
        <div style="font-size:.8rem;color:var(--text-muted);margin-top:.2rem">${sectors.length} sectores Â· ${tempSearchResults.length} empresas unicas Â· ${newCount} nuevas Â· ${duplicates} duplicados fusionados</div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:.5rem;margin-top:.85rem">
      ${sectors.map(seg => {
        ...
        return `<div style="padding:.65rem;border:1px solid var(--glass-border);border-radius:10px;background:rgba(255,255,255,.035)">
          ...
          <div style="font-size:.7rem;color:var(--text-muted);margin-top:.25rem">${s.email} email Â· ${s.ready} listos Â· ${s.pain} dolores Â· score ${s.avg}</div>
...
function getDataTrust(c) {
  ...
  const items = [];
  if (c.email) {
    const src = c.contactEmailRole ? `${c.contactEmailRole} Â· ${c.contactEmailReason || 'email validado'}` : (c.emailCandidates?.[0]?.reason || (c.emailQuality ? `calidad ${c.emailQuality}` : 'email validado'));
...
function buildResultExplanation(c = {}) {
  ...
  if (c.searchPoint || c.radiusUsed) items.push({ label: 'Zona', value: [c.searchPoint, c.radiusUsed ? `${c.radiusUsed}km` : ''].filter(Boolean).join(' Â· '), ok: true });
  ...
  items.push({ label: 'Email', value: c.email ? `${c.contactEmailRole || 'email'} Â· ${c.contactQuality}` : 'sin email', ok: !!c.email });
...
async function waitForGoogleMaps(timeoutMs = 15000) {
  const apiKey = localStorage.getItem('gordi_api_key');
  if (!apiKey) {
    throw new Error('API Key de Google no configurada. Ve a Configuracion -> API Keys.');
  }

  if (!document.getElementById('google-maps-script') && !(window.google && google.maps && google.maps.importLibrary)) {
    loadGoogleMapsScript(apiKey);
  }

  const start = Date.now();
  while (Date.now() - start <= timeoutMs) {
    if (window.GORDI_GOOGLE_MAPS_LOAD_ERROR) {
      throw new Error(window.GORDI_GOOGLE_MAPS_LOAD_ERROR);
    }
    if (typeof google !== 'undefined' && google.maps && typeof google.maps.importLibrary === 'function') {
      try {
        ...
        throw new Error(formatGoogleMapsLoadError(err));
      }
    }
    await sleep(200);
  }

  throw new Error('Google Maps no termino de cargar. Revisa conexion, restricciones HTTP referrer de la key, Maps JavaScript API y Places API (New).');
}

function formatGoogleMapsLoadError(err) {
  const msg = err?.message || String(err || 'error desconocido');
  if (/RefererNotAllowedMapError|referer|referrer/i.test(msg)) {
    return 'La API Key de Google no permite este dominio/origen. Anade la URL exacta en restricciones HTTP referrer.';
  }
  if (/ApiNotActivatedMapError|not activated|disabled/i.test(msg)) {
    return 'Falta activar Maps JavaScript API y/o Places API (New) en Google Cloud.';
  }
  if (/InvalidKeyMapError|key/i.test(msg)) {
    return 'La API Key de Google no es valida o no esta autorizada para Maps JavaScript API.';
  }
  if (/Quota|OVER_QUERY_LIMIT|RESOURCE_EXHAUSTED|billing/i.test(msg)) {
    return 'Google ha rechazado la peticion por cuota, facturacion o limite del proyecto.';
  }
  return msg;
}
...
async function buildSearchPlan(segment, location, maxResults, opts = {}) {
  ...
  return {
    ...
    summary: `${kind} Â· ${points.length} zonas Â· ${allQueries.length} queries${isMulti ? ' Â· multi-sector' : ''}`,
  };
}
      ...
      for (const query of batch.queries) {
        ...
        try {
          ...
          if (exhaustive || effectiveMax > 100) {
            logEnrich(`  -> ${allPlaces.length} empresas Ãºnicas encontradas...`);
          }
...
//   Â· Con email confirmado -> 30 dÃ­as (dato fiable, raramente cambia)
//   Â· Enriquecido pero sin email -> 7 dÃ­as (igual que antes)
//   Â· Sin enriquecer / fallÃ³ -> 2 dÃ­as (vale la pena reintentar pronto)
...
function extractDecisionMakerAdvanced(html = '', sourceUrl = '') {
  ...
  const namePattern = '([A-ZÃÃ‰ÃÃ“ÃšÃ‘][a-zÃ¡Ã©Ã­Ã³ÃºÃ±A-ZÃÃ‰ÃÃ“ÃšÃ‘]+(?:\s+[A-ZÃÃ‰ÃÃ“ÃšÃ‘][a-zÃ¡Ã©Ã­Ã³ÃºÃ±A-ZÃÃ‰ÃÃ“ÃšÃ‘]+){1,3})';
  const patterns = [
...
async function enrichFromWeb(company) {
  ...
  if (!company.decision_maker) {
    ...
    const namePattern = /([A-ZÃÃ‰ÃÃ“ÃšÃ‘][a-zÃ¡Ã©Ã­Ã³ÃºÃ±]{1,20}(?:\s+[A-ZÃÃ‰ÃÃ“ÃšÃ‘][a-zÃ¡Ã©Ã­Ã³ÃºÃ±]{1,20}){1,2})/
    const roleStr = getRoleRegexSource();
...
async function extractEmailWithAI(websiteUrl, companyName, geminiKey) {
  ...
  try {
    ...
    // Limpiar HTML -> solo texto visible (mÃ¡ximo 3000 chars para no gastar tokens)
...
async function enrichFromReviews(company) {
  ...
  try {
    ...
    if (reviews._stats?.ratingTrend) {
      ...
      if (delta <= -0.4)
        company.signals.push(`ðŸ“‰ Rating cayendo: ${avgOld}â˜… -> ${avgRecent}â˜… en Ãºltimos meses â€” urgencia alta`);
      else if (delta >= 0.4)
        company.signals.push(`ðŸ“ˆ Rating mejorando: ${avgOld}â˜… -> ${avgRecent}â˜… â€” puede estar recuperÃ¡ndose`);
    }
...
async function searchBusinessesSingle(options = {}) {
  ...
  if (enrichMode === 'all' || enrichMode === 'web') {
    ...
    logEnrich(`  -> Procesando en orden de potencial (rating + reseÃ±as + web)`);
    ...
    for (let b = 0; b < enrichOrder.length; b += BATCH_SIZE) {
      ...
      batchIndices.forEach(i => {
        ...
        logEnrich(`  -> ${c.name}: ${c.email ? 'âœ‰ï¸ ' + c.email : 'â€”'}${proxyFail ? ' âš ï¸retry-needed' : ''}`);
      });
...
function loadGoogleMapsScript(apiKey) {
  ...
  window.GORDI_GOOGLE_MAPS_LOAD_ERROR = '';
  window.__gordiGoogleMapsReady = false;
  window.__gordiGoogleMapsCallback = function() {
    window.__gordiGoogleMapsReady = true;
  };
  const script = document.createElement('script');
  script.id = 'google-maps-script';
  const params = new URLSearchParams({
    key: String(apiKey || ''),
    v: window.GORDI_GOOGLE_MAPS_VERSION || 'quarterly',
    libraries: 'places,geocoding',
    language: 'es',
    region: 'ES',
    loading: 'async',
    callback: '__gordiGoogleMapsCallback'
  });
  script.src = 'https://maps.googleapis.com/maps/api/js?' + params.toString();
  script.async = true;
  script.onerror = function() {
    window.GORDI_GOOGLE_MAPS_LOAD_ERROR = 'Google Maps no pudo cargarse. Revisa conexion, API key, facturacion y restricciones HTTP referrer.';
  };
...
(function patchExtendedSegmentQueries() {
  ...
  // en Google Places API (textQuery). Orden: mÃ¡s probable -> menos probable.
